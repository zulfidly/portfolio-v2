...
export const useMainStorePinia = defineStore("appStore", {
  ...
  getters: {
    // computed (eager to execute), state may not update although reactive; think isMobile
    isDesktop: (state) =>
      state.clientScr.formFactor === "Desktop|Laptop" ? true : false,
    isPortrait: (state) =>
      state.clientScr.orientation === "portrait-primary" ||
      state.clientScr.orientation === "portrait-secondary"
        ? true
        : false,
    // does not mutate original state
  },
  actions: {
    // function(); will mutate original state
    m_clientScrW() {
      this.clientScr.innerW = window.innerWidth
    },
    m_clientScrH() {
      this.clientScr.innerH = window.innerHeight
    },
    m_clientScrOrientation() {
      this.clientScr.orientation = window.screen.orientation.type
    },
    m_clientScrRatioWH() {
      this.clientScr.ratioWH =
        (window.screen.width / window.screen.height).toFixed(2) * 1
    },
    m_clientScrFormFactor() {
      this.clientScr.formFactor = getUserDeviceFormFactor()
    },
    m_clientScrIsMobile() {
      this.clientScr.isMobile =
        this.clientScr.formFactor === "Smartphone" ||
        this.clientScr.formFactor === "Tablet"
          ? true
          : false
    },
    m_clientScrIsMobileAndLandscape() {
      this.clientScr.isMobileAndLandscape = getIsMobileAndLandscapeStatus()
    },
    m_clientScrIsDarkDevice() {
      this.clientScr.isDarkDevice = getIsDarkDeviceStatus()
    },
    client_IsDarkViewport(bool) {
      this.clientApp.isDarkViewport = bool
    },
    client_updateCurrentPath(path) {
      this.clientApp.currPath = path
    },
    client_socMedVisibilityMngr(bool) {
      this.clientApp.isSocMedHidden = bool
    },
  },
})
...
function getUserDeviceFormFactor() {
  ...
  if (
    ...
    if (window.screen.width < 1024) {
      ...
      // if width > 1024px in landscape i.e: laptops or normal horizontal desktop screens
      return "Desktop|Laptop"
    }
