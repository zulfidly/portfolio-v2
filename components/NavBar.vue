<script setup>
    ...
    const nav = {
        svg: {
            init: ["transition-all duration-[500ms]"],
        ...
        menu: {
            ctnr: {
                ...
                close: ["right-full delay-300"],
                open: ["right-[90%]"]
            },
            init: ["text-6xl md:text-3xl transition-all duration-300"],
            close: ["opacity-0 -translate-x-0"],
            open: ["opacity-100 translate-x-full"],
        },
        closeSVG: {
            init: [" transition-all duration-300 fill-[var(--color-text)]"],
        }
    }
    const navSensor = {
        ...
        in: ["left-0"],
        out: ["-left-full"],
        ctnr: ["fixed w-[55px] h-[76px] -translate-y-1/2 top-1/2 transition-all duration-300"]
    }
    const navBtn = {
        init: ["fixed  -translate-y-1/2 top-1/2 transition-all duration-300"],
        hide: ["-left-full"],
        show: ["left-0"]
    }
...
<template>
    <button :class="[navBtn.init, isSensorHidden?navBtn.show:navBtn.hide]" @click="toggleMenu">
    ...
    <div :class="[navSensor.ctnr, isSensorHidden?navSensor.out:navSensor.in]"  @click="slideInNavIcon">
	...
	<div :class="[nav.menu.ctnr.init, isMenuHidden?nav.menu.ctnr.close:nav.menu.ctnr.open]">
		<NuxtLink @mousedown="toggleMenu" to="/">           <p :class="link" class="delay-[50ms]" >Home</p></NuxtLink>
		<NuxtLink @mousedown="toggleMenu" to="/projects">   <p :class="link" class="delay-[100ms]">Projects</p></NuxtLink>
		<NuxtLink @mousedown="toggleMenu" to="/about">      <p :class="link" class="delay-[150ms]">About</p></NuxtLink>
		<NuxtLink @mousedown="toggleMenu" to="/contact">    <p :class="link" class="delay-[200ms]">Contact</p></NuxtLink>

        <button @click="toggleMenu" class="pt-8 flex justify-center delay-[250ms]"  :class="[nav.closeSVG.init, isMenuHidden?nav.menu.close:nav.menu.open]">
