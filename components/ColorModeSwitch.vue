<script setup>
    ...
    const loca = computed(()=> {
        if(props.isMobile) {
            if(props.currentPath === '/projects') return ['absolute bottom-[80px] right-[40px]']
            else return ['absolute m-4 top-0 right-0 ']
        } else {        // for Desktop|Laptop
            return ['absolute mt-8 top-0 right-0 ']
        }
    })

    function colorModeSwitchToggle() {
        document.querySelector('html').classList.toggle('dark')
        emiT('userToggleColorMode', !props.isDarkViewport)        
    }
    function singularColorModeCheckAndToggle(is_dark_bool) {
        if(is_dark_bool) {
    ...
    // parent side codes=> @user-toggle-color-mode="(bool)=> appStore.client_IsDarkViewport(bool)"
</script>

<template>
    <div class="sm:hidden lg:block rounded-xl" :class="[loca]">
        <button class="relative rounded-[11px] block w-[40px] h-[22px] border border-[var(--color-border)] bg-[var(--color-background-mute)] transition
            lg:hover:border lg:hover:border-[var(--color-soft-oppo)]"
            type="button" role="switch" aria-label="Toggle dark mode"
            @click="colorModeSwitchToggle"
            :aria-checked="isDarkViewport">
            <span class="shadow-color-mode-sw dark:translate-x-[18px] absolute top-[1px] left-[1px] w-[18px] h-[18px] rounded-[50%] bg-[var(--color-background)] transition duration-200">
                <span class="relative block rounded-[50%] w-[28px] h-[28px] overflow-hidden box-border">

                    <svg xmlns="http://www.w3.org/2000/svg" aria-hidden="false" focusable="false" viewBox="0 0 24 24" 
                        class="dark:opacity-0 opacity-100 absolute top-[3px] left-[3px] w-[12px] h-[12px] fill-[var(--color-text)] transition-opacity">
                    ...
                    <svg xmlns="http://www.w3.org/2000/svg" aria-hidden="false" focusable="false" viewBox="0 0 24 24" 
                        class="dark:opacity-100 opacity-0 absolute top-[3px] left-[3px] w-[12px] h-[12px] fill-[var(--color-text)] transition-opacity">
