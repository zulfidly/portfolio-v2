...
<template>
    <div class="mt-8 lg:mt-0 px-2 py-2 lg:max-w-[55vw] lg:h-[80svh] transition-opacity duration-200" 
        :class="[isMenuHidden?opac.show:opac.hide]">
        ...
        <ul class="grid grid-cols-1 gap-3 h-[inherit] overflow-scroll">
            <li v-for="(stack, ind) in stacks" :key="'stack'+ind" :class="['px-4 py-4 flex flex-col lg:flex-row-reverse items-center justify-between border border-[var(--color-border)] bg-[var(--color-background-soft)] rounded-xl']">
                ...
                <div class="p-4">
                    <section>
                        <a :href="stack.link" target="_blank" :aria-label="'visit '+stack.title+' website'" class="text-lg lg:text-bold font-medium underline decoration-dotted underline-offset-4 text-[var(--color-text)]">
                            {{ stack.title }}</a>
